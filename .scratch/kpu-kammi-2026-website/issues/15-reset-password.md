...
- Diperiksa dan ditutup 2026-09-16. Kode dicocokkan dengan tiap butir: alfabet 56 karakter tanpa `0 O o 1 l I`, penolakan byte ≥ 224 (tanpa bias modulo), penghitung `konfirmasi:<HMAC sesiId>` direset saat berhasil dan mencabut sesi Admin pada kegagalan kelima, target dibatasi `role = 'bacalon'`, audit `reset_kata_sandi` berhasil/gagal tanpa kata sandi. Dialog klien menampilkan kata sandi sekali dengan tombol salin. `npm test` 118/118 lulus (turun dari 123 karena `e5d510f` sengaja membuang uji `PUT /api/admin/peraturan`; 36 unhandled rejection adalah cacat Better Auth 1.7 yang terdokumentasi di `vitest.config.ts`, exit 0), `npx tsc -b` bersih, `npx eslint .` 0 error. Walkthrough peramban tidak diulang; perilaku server dibuktikan oleh uji seam Worker.
