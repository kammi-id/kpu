...
export function AdminPeraturan() {
	...
	return (
		<div className="grid gap-10">
			<div>
				<h2 className="font-display text-3xl text-navy">Peraturan dan Berkas Publik</h2>
				<p className="mt-2 text-muted-foreground">Kelola ringkasan PKPU, dokumen Peraturan, dan Formulir A.1–A.6.</p>
			</div>
			...
			<form className="grid gap-6 lg:grid-cols-2" onSubmit={simpanMarkdown}>
				<fieldset className="flex flex-col gap-2">
					<label htmlFor="isi-markdown" className="font-semibold text-navy">Ringkasan PKPU (Markdown)</label>
					<textarea
						id="isi-markdown"
						name="isiMarkdown"
						value={markdown}
						...
						maxLength={400000}
						className="min-h-80 w-full rounded-xl border border-input bg-white px-3 py-2 text-base leading-relaxed outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30"
						aria-describedby="batas-markdown"
					/>
				...
				<section aria-labelledby="pratinjau-peraturan" className="rounded-2xl border border-border bg-white p-5">
					<h3 id="pratinjau-peraturan" className="font-display text-xl text-navy">Pratinjau</h3>
					<div className="mt-4">
						{markdown ? <SafeMarkdown>{markdown}</SafeMarkdown> : <p className="text-muted-foreground">Menyusul.</p>}
					</div>
				</section>
			</form>
			<form className="grid gap-4 rounded-2xl border border-border bg-white p-5" onSubmit={unggah}>
				...
				<div className="grid gap-4 sm:grid-cols-2">
					<fieldset className="flex flex-col gap-2">
						<label htmlFor="judul" className="font-semibold">Judul</label>
						<Input id="judul" name="judul" required />
					</fieldset>
					<fieldset className="flex flex-col gap-2">
						<label htmlFor="kategori" className="font-semibold">Kategori</label>
						<select id="kategori" name="kategori" defaultValue="peraturan" className="h-10 rounded-xl border border-input bg-white px-3" required>
							<option value="peraturan">Peraturan</option>
							<option value="formulir">Formulir</option>
						</select>
					</fieldset>
					<fieldset className="flex flex-col gap-2">
						<label htmlFor="urutan" className="font-semibold">Urutan</label>
						<Input id="urutan" name="urutan" type="number" min="0" step="1" defaultValue="1" required />
					</fieldset>
					<fieldset className="flex flex-col gap-2">
						<label htmlFor="berkas" className="font-semibold">Berkas PDF atau DOCX</label>
						<Input
							id="berkas"
							name="berkas"
							type="file"
			...
			<section aria-labelledby="daftar-berkas">
				<h3 id="daftar-berkas" className="font-display text-2xl text-navy">Berkas tersimpan</h3>
				<DaftarAdmin judul="Peraturan" berkas={peraturan} onHapus={hapus} disabled={menyimpan} />
				<DaftarAdmin judul="Formulir" berkas={formulir} onHapus={hapus} disabled={menyimpan} />
			</section>
		</div>
	);
}

type PropsDaftarAdmin = {
	judul: string;
	berkas: BerkasPublik[];
...
function DaftarAdmin({ judul, berkas, onHapus, disabled }: PropsDaftarAdmin) {
	return (
				...
				<ul className="mt-2 divide-y divide-border rounded-xl border border-border">
					{berkas.map((item) => (
						<li key={item.id} className="flex items-center justify-between gap-3 px-4 py-3">
							<span>
								<span className="block font-medium">{item.judul}</span>
								<span className="block text-sm text-muted-foreground">{item.namaAsli}</span>
							</span>
							<Button type="button" variant="destructive" size="sm" disabled={disabled} onClick={() => void onHapus(item.id)}>
								Hapus
							</Button>
						</li>
					))}
